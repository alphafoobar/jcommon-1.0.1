...
public class DayOfWeekInMonthRule extends AnnualDateRule {
    ...
    public SerialDate getDate(int year) {
        if (this.count != SerialDateImpl.LAST_WEEK_IN_MONTH) {
            // start at the beginning of the month
            SerialDate result = SerialDateImpl.createInstance(1, this.month, year);
            while (result.getDayOfWeek() != this.dayOfWeek) {
                result = SerialDateImpl.addDays(1, result);
            }
            return SerialDateImpl.addDays(7 * (this.count - 1), result);
        }
        // start at the end of the month and work backwards...
        SerialDateImpl result = SerialDateImpl.createInstance(1, this.month, year);
